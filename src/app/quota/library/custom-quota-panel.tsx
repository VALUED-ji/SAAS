...
import { ChevronDown, Gavel, Pencil, Plus, Search, SlidersHorizontal, Trash2, X } from "lucide-react";
import DataPagination, { useDataPagination } from "@/components/ui/DataPagination";
import ThinScrollArea from "@/components/ui/ThinScrollArea";
import SystemSelect from "@/components/ui/SystemSelect";
import { COMMON_QUOTA_UNITS } from "./quota-library-shared";
import {
  CustomQuotaItem,
  PromoteConfirmState,
  formatAmount,
...
export function CustomQuotaLibraryPanel({
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter((item) => {
      ...
      return matchesKeyword && matchesStore && matchesCategory;
    });
  }, [categoryFilter, items, search, storeFilter]);
  const pagination = useDataPagination(filteredItems, [search, storeFilter, categoryFilter].join("|"));
  ...
  const deleteCustomItem = async (item: CustomQuotaItem) => {
    if (!window.confirm(`确认删除自定义项目“${item.name}”吗？`)) return;
    try {
      await requestCustomQuotaLibrary("", {
        method: "POST",
        body: JSON.stringify({ action: "delete", id: item.id }),
      });
      onNotice(`已删除自定义项目：${item.name}`);
      await loadItems();
    } catch (error: any) {
  ...
  return (
    <section className="table-shell quota-list-shell flex min-h-0 flex-1 flex-col">
      ...
      <ThinScrollArea className="quota-list-scroll min-h-0 flex-1" scrollClassName="quota-list-scroll-region h-full overflow-auto">
        <table className="quota-list-table w-full min-w-[1320px] table-fixed border-separate border-spacing-0 text-sm [&_td]:border-b [&_td]:border-surface-100 [&_td:not(:last-child)]:border-r [&_td:not(:last-child)]:border-surface-100 [&_th]:border-b [&_th]:border-surface-300 [&_th:not(:last-child)]:border-r [&_th:not(:last-child)]:border-surface-200">
          <colgroup>
            <col className="w-[8%]" />
            <col className="w-[7%]" />
            <col className="w-[15%]" />
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={10} className="px-4 py-12 text-center text-sm text-surface-500">正在加载自定义库...</td></tr>
              ...
              <tr key={item.id} className="bg-white hover:bg-surface-50">
                <td className="quota-scope-cell px-3 py-3 text-center text-surface-700">{item.storeName || "-"}</td>
                <td className="quota-category-cell px-3 py-3 text-center text-surface-700">{item.category || "未分类"}</td>
                ...
                <td className="quota-source-cell px-3 py-3 text-center text-surface-700">
                  <span className="inline-flex max-w-full truncate" title={item.sourceEmployeeName || item.createdByName || "-"}>
                    {item.sourceEmployeeName || item.createdByName || "-"}
                  </span>
                </td>
                <td className="px-3 py-3 text-center">
                  <div className="quota-row-actions quota-custom-actions inline-flex flex-nowrap items-center justify-center gap-1">
                    ...
                    {item.status !== "promoted" && (
                      <button type="button" onClick={() => promoteItem(item)} className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-primary-100 bg-white text-primary-700 transition hover:border-primary-200 hover:bg-primary-50" title="转正到标准定额库" aria-label="转正到标准定额库">
                        <Gavel className="h-3.5 w-3.5" />
                      </button>
                    )}
                    <button type="button" onClick={() => deleteCustomItem(item)} className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-red-100 bg-white text-red-500 transition hover:border-red-200 hover:bg-red-50 hover:text-red-600" title="删除自定义项目">
      ...
      {editingItem && typeof document !== "undefined" && createPortal(
        <div
          className="quota-management-ui quota-library-page qm-modal-overlay quota-custom-editor-overlay fixed inset-y-0 left-[var(--active-sidebar-width,260px)] right-0 z-50 flex items-center justify-center px-6 py-7 max-md:inset-0 max-md:px-4"
          onMouseDown={(event) => {
            if (event.target === event.currentTarget) setEditingItem(null);
          }}
        >
          <div role="dialog" aria-modal="true" className="qm-modal-shell qm-editor-modal quota-editor-redesign quota-custom-editor-modal flex w-full max-w-5xl flex-col overflow-hidden border border-surface-200 bg-white">
            ...
            <div className="qm-modal-body qm-editor-form quota-editor-body overflow-y-auto px-6 py-5">
              ...
              <div className="quota-editor-flow quota-custom-editor-flow">
              ...
              <section className="quota-editor-section quota-editor-meta-section quota-custom-editor-meta-section">
                <div className="quota-editor-section-head">
                  <b className="quota-editor-step-index">2</b>
                  <span>归属信息</span>
                  <small>用于转正后的分类、工种和材料归集</small>
                </div>
                <div className="quota-editor-meta-grid">
                  <label className="qm-field quota-editor-field">
                    <span>工种</span>
                    <SystemSelect
